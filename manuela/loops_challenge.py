...
less_than_50_message = 'Put on your jacket! '

#create message variable for temp > 50 < 65
between_50_and_65 = 'Put on your sweater. '

#create message variable for temp >=65
above_65 = 'Apply sunscreen, now! '

# create a loop that loops through the list days of the week asking input of temp that day. Make sure input is an integer
for weekday in days:
    current_temp = int(input(f'Today is {weekday}. What is the temperature today? '))
    #create if-else to choose above appropriate message given input
    if current_temp < 50:
...
happy_birthday = [1, 2, 5, 10]
                  # 0  1   2  3
#for times_of_wishes in happy_birthday:
   # print(times_of_wishes)
    
for i in range (4):
    print(happy_birthday)

wishes = int(input(f'How many times do you want to be wished happy birthday {happy_birthday[i]}?'))
print(f'Number of times{wishes[i]} you want to be wished happy birthday {happy_birthday[i]}?')
