...
for laptop in laptops:
    ...
    if laptop["productName"] == "Apple Macbook Pro":
...
try:
    ...
    # THERE IS NO "1 TB" on the 13-inch MacBook Pro
    laptops[0]["types"][0]["storage"].remove("1 TB SSD")
    # INCASE ITS MEANT FOR THE 16-inch
...
for laptop in laptops:
    # try and loop through types key
    try:
        for type in laptop['types']:
            # Try and loop through price key
            try:
                for price in type['price']:
                    print(price)
            except:
